Room.prototype.runHarvester = function(creep) {
  ...
  var source_withdraw = creep.room.find(FIND_STRUCTURES, {
    filter: (structure) => {
      return (structure.structureType == STRUCTURE_CONTAINER) && structure.store[RESOURCE_ENERGY] > 0;
    }
  });
  var main_withdraw = creep.pos.findClosestByPath(source_withdraw);
  var source_deposit = creep.room.find(FIND_STRUCTURES, {
    filter: (structure) => {
      return (structure.structureType == STRUCTURE_CONTAINER || structure.structureType == STRUCTURE_STORAGE) && structure.store[RESOURCE_ENERGY] < structure.storeCapacity || structure.energy < structure.energyCapacity;
    }
  });
